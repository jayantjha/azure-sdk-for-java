...
public final class ThreadsAsyncClient {
     ...
     */
    ThreadsAsyncClient(ThreadsImpl serviceClient, ThreadsClientTracer clientTracer) {
    ...
    public Mono<PersistentAgentThread> createThread(List<ThreadMessageOptions> messages, ToolResources toolResources,
        ...
        final ClientTracer.Operation<Mono<PersistentAgentThread>> operation
            = (arg) -> createThreadWithResponse(createThreadRequest, arg).flatMap(FluxUtil::toMono)
                .map(protocolMethodData -> protocolMethodData.toObject(PersistentAgentThread.class));
        return clientTracer.traceCreateThreadAsync(messages, toolResources, operation, requestOptions);
    }
    ...
    public Mono<PersistentAgentThread> createThread() {
        ...
        final ClientTracer.Operation<Mono<PersistentAgentThread>> operation
            = (arg) -> createThreadWithResponse(createThreadRequest, arg).flatMap(FluxUtil::toMono)
                .map(protocolMethodData -> protocolMethodData.toObject(PersistentAgentThread.class));
        return clientTracer.traceCreateThreadAsync(null, null, operation, requestOptions);
    }
